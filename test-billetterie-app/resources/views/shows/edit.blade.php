        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('shows.update', $show) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="title" class="block text-gray-700 text-sm font-bold mb-2">Titre *</label>
                            <input type="text" name="title" id="title" value="{{ old('title', $show->title) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror">{{ old('description', $show->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="show_date" class="block text-gray-700 text-sm font-bold mb-2">Date du spectacle
                                *</label>
                            <input type="datetime-local" name="show_date" id="show_date"
                                value="{{ old('show_date', $show->show_date->format('Y-m-d\TH:i')) }}"
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="duration" id="duration"
                                    value="{{ old('duration', $show->duration) }}" min="1"
                            ...
                            <div>
                                ...
                                <input type="number" name="price" id="price"
                                    value="{{ old('price', $show->price) }}" step="0.01" min="0"
